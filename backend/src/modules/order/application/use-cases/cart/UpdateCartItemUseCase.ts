...
export class UpdateCartItemUseCase {
  ...
  async execute(userId: string, dto: UpdateCartItemInput): Promise<Order> {
    ...
    const itemIndex = cart.items.findIndex((item: any) => item.productId === dto.productId);
    
    if (itemIndex === -1) {
      throw new NotFoundError(`Product ${dto.productId} not found in cart`, 'CartItem');
    }
    ...
    if (productData.stockQuantity < dto.quantity) {
      throw new ValidationError(
        `Insufficient stock for "${productData.name}". Available: ${productData.stockQuantity}`
      );
    }
